 ...
 *
 * Note: every concurrent slot MUTATION is one 128-bit CAS (mixed-width writes are
 *       forbidden by the protocol). An empty slot is claimed by CAS'ing the exact
 *       observed 128-bit value to {entry, hash, 0}; a loser re-reads the SAME slot,
 ...
 *       The observation is two 8-byte atomic loads, entry half FIRST (see the
 *       in-loop comment for why that order is load-bearing); GCC emits no inline
 *       lock-free 128-bit atomic load, and the branches that mutate always
 *       revalidate the whole observation through the CAS.
 */
#if defined (__x86_64__)
