...
net_histo_setup_names (void)
{
  ...
  net_Req_buffer[NET_SERVER_BO_NOTIFY_HA_LOG_APPLIER_STATE].name =
    "NET_SERVER_BO_NOTIFY_HA_LOG_APPLIER_STATE";
  net_Req_buffer[NET_SERVER_BO_COMPACT_DB].name = "NET_SERVER_BO_COMPACT_DB";
  net_Req_buffer[NET_SERVER_BO_HEAP_COMPACT].name =
    "NET_SERVER_BO_HEAP_COMPACT";
  net_Req_buffer[NET_SERVER_BO_COMPACT_DB_START].name =
  ...
  net_Req_buffer[NET_SERVER_LC_CHECK_FK_VALIDITY].name =
  ...
  net_Req_buffer[NET_SERVER_HEAP_RECLAIM_ADDRESSES].name =
  ...
  net_Req_buffer[NET_SERVER_BTREE_GET_KEY_TYPE].name =
  ...
  net_Req_buffer[NET_SERVER_REPL_BTREE_FIND_UNIQUE].name =
  ...
  net_Req_buffer[NET_SERVER_SES_RESET_CUR_INSERT_ID].name =
  ...
  net_Req_buffer[NET_SERVER_UPDATE_DROP_CLS_BTID].name =
...
net_client_request_no_reply (int request, char *argbuf, int argsize)
{
  ...
  rc = css_send_req_to_server_no_reply (net_Server_host, request, argbuf,
					argsize);
  if (rc == 0)
    {
      error = css_Errno;
 ...
 */
static int
net_client_request_internal (int request, char *argbuf,
  ...
  if (net_Server_name[0] == '\0')
      ...
      else
	{
	  ...
	  if (error != NO_ERROR)
	    ...
	    {
	      error = COMPARE_SIZE_AND_BUFFER (&replysize1, size,
					       &reply1, reply);
	    }
	  ...
	  if (p2_size > 0)
		...
		{
		  error = COMPARE_SIZE_AND_BUFFER (&replysize2, size,
						   &reply2, reply);
		}
	    }
	}
      ...
      do
	{
	  ...
	  switch (server_request)
	    ...
	    case QUERY_END:
	      ...
	      if (reply_datasize_listid + reply_datasize_page +
		  reply_datasize_plan)
		{
		  if ((error == NO_ERROR)
		      && (replydata =
			  (char *) malloc (reply_datasize_listid)) != NULL)
		    {
		      css_queue_receive_data_buffer (rc, replydata,
						     reply_datasize_listid);
		      error =
			css_receive_data_from_server (rc, &reply, &size);
		      if (error != NO_ERROR)
			{
			...
			{
			  error =
			    COMPARE_SIZE_AND_BUFFER (&reply_datasize_listid,
	      ...
	      if (reply_datasize_page + reply_datasize_plan)
		{
		  if ((error == NO_ERROR)
			...
			{
			  error =
			    COMPARE_SIZE_AND_BUFFER (&reply_datasize_page,
						     size, &replydata, reply);
			}
	      ...
	      if (reply_datasize_plan)
		{
		  if ((error == NO_ERROR)
		      && (replydata =
			  (char *) malloc (reply_datasize_plan + 1)) != NULL)
		    {
		      css_queue_receive_data_buffer (rc, replydata,
						     reply_datasize_plan);
		      error =
			...
			{
			  error =
			    COMPARE_SIZE_AND_BUFFER (&reply_datasize_plan,
						     size, &replydata, reply);
			}
		...
		if (methoddata != NULL)
		  {
			...
			error = COMPARE_SIZE_AND_BUFFER (&methoddata_size,
							 ...
							 &methoddata, reply);
			ptr =
			  or_unpack_unbound_listid (methoddata,
						    (void **)
						    &method_call_list_id);
			     ...
			     char **logpg_area_buf, bool verbose)
{
  ...
  if (net_Server_name[0] == '\0')
      ...
      if (ctx_ptr->rc == -1)
	{
	  /* HEADER PAGE REQUEST */
	  rc =
	    css_send_req_to_server_2_data (net_Server_host, request,
					   argbuf, argsize, NULL, 0,
 ...
 */
int
net_client_request_with_logwr_context (LOGWR_CONTEXT * ctx_ptr,
  ...
  if (net_Server_name[0] == '\0')
      ...
      do
	{
	  ...
	  switch (server_request)
		...
		ptr = or_unpack_int (ptr, (int *) (&length));
		error =
		  net_client_get_next_log_pages (rc, replybuf,
						 replysize, length);
	      }
	      break;
	    case END_CALLBACK:
 ...
 */
int
net_client_get_next_log_pages (int rc, char *replybuf, int replysize,
			       int length)
{
  char *reply = NULL;
  int error;
...
#endif /* HISTO */
      rc =
	css_send_req_to_server (net_Server_host, request, argbuf,
				argsize, databuf, datasize, replybuf,
				replysize);
      if (rc == 0)
	{
				  ...
				  LC_COPYAREA ** reply_copy_area)
{
  ...
  if (net_Server_name[0] == '\0')
      ...
      if (net_Histo_setup)
	{
      ...
      rc =
	css_send_req_to_server (net_Server_host, request, argbuf,
				argsize, NULL, 0, replybuf, replysize);
      if (rc == 0)
	{
	  return set_server_error (css_Errno);
	}
      ...
      if (packed_desc_size != 0 || content_size != 0)
	{
	  if (error == NO_ERROR && reply_copy_area != NULL)
	    {
	      *reply_copy_area =
		locator_recv_allocate_copyarea (num_objs, &packed_desc,
						packed_desc_size,
				 ...
				 int replysize, LOG_COPY ** reply_log_area)
{
  ...
  *reply_log_area = NULL;
  ...
  if (net_Histo_setup)
    {
      net_histo_request_finished (request, replysize + content_size +
				  packed_desc_size);
    }
#endif /* HISTO */
 ...
 */
int
net_client_request_2recv_copyarea (int request, char *argbuf,
				   int argsize, char *replybuf,
				   int replysize, char *databuf,
 ...
 */
int
net_client_request_3recv_copyarea (int request, char *argbuf,
				   int argsize, char *replybuf,
				   int replysize, char *databuf,
				   int datasize, char **recvbuffer,
				   int *recvbuffer_size,
 ...
 */
int
net_client_request_recv_stream (int request, char *argbuf,
				int argsize, char *replybuf,
...
net_client_ping_server (int client_val, int *server_val, int timeout)
{
  ...
  error =
    css_receive_data_from_server_with_timeout (eid, &reply, &reply_size,
					       timeout);
  if (error || reply == NULL)
    {
