	  ...
	  if (manager.px_partition_loop)
	    {
	      ...
	      if (thread_is_on_trace (thread_p))
		{
		  /* the px executor fills stats->parallel inside execute_partitions;
		   * fill it here so the trace's PARALLEL line covers the loop too */
		  HASHJOIN_START_STATS parallel_start_stats = HASHJOIN_START_STATS_INITIALIZER;

		  hjoin_trace_start (thread_p, &parallel_start_stats);
		  error = hjoin_execute_partitions (thread_p, &manager);
		  hjoin_trace_end (thread_p, &manager.single_context.stats->parallel, &parallel_start_stats);
		}
	      else
		{
		  error = hjoin_execute_partitions (thread_p, &manager);
 ...
 *       prediction, so a caller that gates on it (the partition parallel build requires
 *       IN_MEM or HYBRID) can never drift from the actual choice.
 */
HASH_METHOD
...
hjoin_trace_merge_stats (HASHJOIN_STATS * stats, HASHJOIN_STATS * context_stats, HASHJOIN_STATUS status)
{
  ...
  if (context_stats->num_parallel_threads > 1)
      ...
      if (stats->num_partition_probe_threads == 0)
	{
	  ...
	  /* the loop executor's BUILD range: per-partition build times (the px
	   * executor fills this field with per-worker totals instead) */
	  stats->build.range_elapsed_time.min = context_stats->build.elapsed_time;
	  stats->build.range_elapsed_time.max = context_stats->build.elapsed_time;
	}
      else
	{
	  perfmon_update_min_timeval (&stats->build.range_elapsed_time.min, &context_stats->build.elapsed_time);
	  perfmon_update_max_timeval (&stats->build.range_elapsed_time.max, &context_stats->build.elapsed_time);
