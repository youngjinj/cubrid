...
do_create_entity (PARSER_CONTEXT * parser, PT_NODE * node)
{
  ...
  switch (node->info.create_entity.entity_type)
    ...
    case PT_CLASS:
      ...
      if (node->info.create_entity.if_not_exists == 1 && db_find_self_class_youngjinj (class_name))
	{
	  goto error_exit;
	}
      ...
      while (super_node)
	{
	  super_class = db_find_self_class_youngjinj (super_node->info.name.original);
