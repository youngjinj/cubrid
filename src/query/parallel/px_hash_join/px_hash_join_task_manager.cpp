...
namespace parallel_query
{
  namespace hash_join
  {
    ...
    build_task::build_task (task_manager &task_manager, HASHJOIN_MANAGER *manager, MHT_HLS_TABLE *table,
      ...
      , m_arena (arena)
      , m_rows_out (rows_out)
      , m_slots_out (slots_out)
      , m_shared_info (shared_info)
    {
      assert (m_table != nullptr && m_arena != 0);
      assert (m_rows_out != nullptr && m_slots_out != nullptr);
      assert (m_shared_info != nullptr);
    ...
    build_task::execute (cubthread::entry &thread_ref)
    {
      ...
      MHT_HLS_ENTRY *entry;
      UINT32 hash_key;
      UINT64 rows = 0, slots = 0;	/* local: the caller-owned counters are adjacent
					 * array elements, so per-tuple increments through
					 * the out-pointers would false-share one cache
					 * line across the workers */
      int tuple_cnt, tuple_index, tuple_length;
      int error = NO_ERROR;
      ...
      do
	{
	  ...
	  do
		...
		{
		  slots++;	/* this insert claimed a fresh slot */
		}
	    }
	  while (true);
      ...
      if (overflow_record.tpl != nullptr)
	{
      ...
      /* single stores; the caller reads them after the join */
      *m_rows_out = rows;
      *m_slots_out = slots;
		  ...
		  if (m_context->after_join_pred != nullptr)
		    {
		      ...
		      HJOIN_PROFILE_START (&thread_ref, &profile_start_stats, HASHJOIN_PROFILE_PROBE_MATCH);
		      ev_res = hjoin_eval_pred (&thread_ref, probe, build, m_context->after_join_pred, m_context->val_descr);
		      HJOIN_PROFILE_END (&thread_ref, &stats->profile, &profile_start_stats, HASHJOIN_PROFILE_PROBE_MATCH);
		  ...
		  if (m_context->during_join_pred != nullptr)
		    {
		      ...
		      HJOIN_PROFILE_START (&thread_ref, &profile_start_stats, HASHJOIN_PROFILE_PROBE_MATCH);
		      ev_res = hjoin_eval_pred (&thread_ref, probe, build, m_context->during_join_pred, m_context->val_descr);
		      HJOIN_PROFILE_END (&thread_ref, &stats->profile, &profile_start_stats, HASHJOIN_PROFILE_PROBE_MATCH);
