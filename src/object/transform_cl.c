...
static DB_LIST *get_substructure_set (OR_BUF * buf, LREADER reader,
				      int expected);
...
tf_need_permanent_oid (OR_BUF * buf, DB_OBJECT * obj)
{
   ...
   * an entry for it.
   */
  if (tf_Allow_fixups && buf->fixups != NULL
      && obj->class_mop != NULL && obj->class_mop != sm_Root_class_mop)
    {
...
tf_mem_to_disk (MOP classmop, MOBJ classobj,
		MOBJ volatile obj, RECDES * record, bool * index_flag)
{
  OR_BUF orep, *buf;
...
tf_class_size (MOBJ classobj)
{
  ...
  size += substructure_set_size ((DB_LIST *) class_->attributes,
				 (LSIZER) attribute_size);
  size += substructure_set_size ((DB_LIST *) class_->shared,
				 (LSIZER) attribute_size);
  size += substructure_set_size ((DB_LIST *) class_->class_attributes,
				 (LSIZER) attribute_size);

  size += substructure_set_size ((DB_LIST *) class_->methods,
				 (LSIZER) method_size);
  size += substructure_set_size ((DB_LIST *) class_->class_methods,
				 (LSIZER) method_size);
...
tf_dump_class_size (MOBJ classobj)
{
  ...
  s = substructure_set_size ((DB_LIST *) class_->shared,
			     (LSIZER) attribute_size);
  fprintf (stdout, "Shared attributes %d\n", s);
  size += s;

  s = substructure_set_size ((DB_LIST *) class_->class_attributes,
			     (LSIZER) attribute_size);
  fprintf (stdout, "Class attributes %d\n", s);
  size += s;

  s = substructure_set_size ((DB_LIST *) class_->methods,
			     (LSIZER) method_size);
  fprintf (stdout, "Methods %d\n", s);
  size += s;

  s = substructure_set_size ((DB_LIST *) class_->class_methods,
			     (LSIZER) method_size);
