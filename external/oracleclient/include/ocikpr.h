...
sword obindps (struct cda_def *cursor, ub1 opcode, OraText * sqlvar,
	       ...
	       ub4 maxsiz, ub4 * cursiz, OraText * fmt, sb4 fmtl, sword fmtt);
...
sword oerhms (struct cda_def *lda, sb2 rcode, OraText * buf, sword bufsiz);
sword oermsg (sb2 rcode, OraText * buf);
sword oexec (struct cda_def *cursor);
sword oexfet (struct cda_def *cursor, ub4 nrows, sword cancel, sword exact);
...
sword olog (struct cda_def *lda, ub1 * hst,
	    ...
	    OraText * psw, sword pswl, OraText * conn, sword connl, ub4 mode);
