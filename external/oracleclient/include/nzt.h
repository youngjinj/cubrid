 ...
 */
nzerror nztwOpenWallet (nzctx *, ub4, text *, ub4, text *, nzttWallet *);
...
nzerror nztwGetCertInfo (nzctx * nz_context,
			 nzosContext * nzosCtx,
			 nzttWallet * walletRef, void *peerCert);
 ...
 *    NZERROR_PARMS     Error in description.
 */
nzerror nztiDuplicateIdentity (nzctx *, nzttIdentity *, nzttIdentity **);
#endif /* NZ_OLD_TOOLS */
 ...
 *    NZERROR_OK     Success.
 */
nzerror nztidGetIdentityDesc (nzctx *, nzttIdentity *, nzttIdentityDesc **);
 ...
 */
nzerror nztific_FreeIdentityContent (nzctx * ossctx, nzttIdentity * identity);
...
nzerror nztVerify (nzctx *, nzttPersona *, nzttces, ub4, ub1 *,
		   nzttBufferBlock *, boolean *, boolean *, nzttIdentity **);
 ...
 *    NZERROR_TK_NOTSUPP   Function not supported with persona.
 */
nzerror nztxkec_PKEncryptExpansion (nzctx *, nzttPersona *, ub4, ub4, ub4 *);
 ...
 *    NZERROR_TK_NOTSUPP   Function not supported with persona.
 */
nzerror nztrb_RandomBytes (nzctx *, nzttPersona *, ub4, nzttBufferBlock *);
 ...
 *    NZERROR_OK        Success.
 */
nzerror nztbbSetBlock (nzctx *, uword, ub4, ub4, ub1 *, nzttBufferBlock *);
...
nzerror nztiGetDName (nzctx *, nzttIdentity *, text **, ub4 *);
 ...
 *    
 */
nzerror nztiGetIssuerName (nzctx *, nzttIdentity *, text **, ub4 *);
 ...
 *    
 */
nzerror nztgch_GetCertHash (nzctx *, nzttIdentity *, ub1 **, ub4 *);
 ...
 */
nzerror nztwCreateWallet (nzctx *, ub4, text *, ub4, text *, nzttWallet *);
 ...
 *    NZERROR_OK     Success.
 */
nzerror nztiStoreTrustedIdentity (nzctx *, nzttIdentity **, nzttPersona *);
...
nzerror nztifdn (nzctx * ossctx,
		 ub4 length,
		 text * distinguished_name, nzttIdentity ** ppidentity);
 ...
 *    NZERROR_TK_NOTSUPP   Function not supported with persona.
 */
nzerror nztxsd_SignDetachedExpansion (nzctx *, nzttPersona *, ub4, ub4 *);
 ...
 *    NZERROR_TK_NOTSUPP   Function not supported with persona.
 */
nzerror nztxKeyedHashExpansion (nzctx *, nzttPersona *, ub4, ub4 *);
...
nzerror nztiae_IsAuthEnabled (nzctx * ctx,
			      ub2 ncipher, boolean * authEnabled);
...
nzerror nztiee_IsEncrEnabled (nzctx * ctx,
			      ub2 ncipher, boolean * EncrEnabled);
...
nzerror nztihe_IsHashEnabled (nzctx * ctx,
			      ub2 ncipher, boolean * hashEnabled);
...
nzerror nztGetIssuerName (nzctx * ctx,
			  ...
			  ub1 ** issuername, ub4 * issuernamelen);

nzerror nztGetSubjectName (nzctx * ctx,
			   nzttIdentity * identity,
			   ub1 ** subjectname, ub4 * subjectnamelen);

nzerror nztGetBase64Cert (nzctx * ctx,
			  nzttIdentity * identity,
			  ub1 ** b64cert, ub4 * b64certlen);

nzerror nztGetSerialNumber (nzctx * ctx,
			    nzttIdentity * identity,
			    ub1 ** serialnum, ub4 * serialnumlen);

nzerror nztGetValidDate (nzctx * ctx,
			 nzttIdentity * identity,
			 ub4 * startdate, ub4 * enddate);

nzerror nztGetVersion (nzctx * ctx,
		       nzttIdentity * identity, nzstrc * pVerStr);

nzerror nztGetPublicKey (nzctx * ctx,
			 nzttIdentity * identity,
			 ub1 ** pubKey, ub4 * pubKeylen);

nzerror nztGenericDestroy (nzctx * ctx, ub1 ** var);

nzerror nztSetAppDefaultLocation (nzctx * ctx, text *, size_t);

nzerror nztSearchNZDefault (nzctx * ctx, boolean * search);
